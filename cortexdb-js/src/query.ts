...
export class QueryAPI {
    ...
    async getContextsByKb(kbId: string): Promise<QueryResponse> {
        return this.http.get<QueryResponse>(`/api/v1/memory/query/contexts/kb/${kbId}`);
    }

    /** Get recent contexts from the last N days. */
    async getRecentContexts(days: number = 7): Promise<QueryResponse> {
        return this.http.get<QueryResponse>("/api/v1/memory/query/contexts/recent", { days });
    }
    ...
    async getSiblingContexts(contextId: string): Promise<QueryResponse> {
        return this.http.get<QueryResponse>(`/api/v1/memory/query/contexts/${contextId}/siblings`);
    }
    ...
    async getEntityByName(name: string): Promise<Entity | null> {
        return this.http.get<Entity>(`/api/v1/memory/query/entities/name/${name}`);
    }
    ...
    async getEntityByNameIgnoreCase(name: string): Promise<Entity | null> {
        return this.http.get<Entity>(`/api/v1/memory/query/entities/name/${name}/ignorecase`);
    }

    /** Get entity ID by name. Returns null if not found. */
    async getEntityIdByName(name: string): Promise<string | null> {
        const data = await this.http.get<{ id: string }>(`/api/v1/memory/query/entities/id/${name}`);
        return data?.id ?? null;
    }
    ...
    async getContextsForEntity(entityId: string): Promise<any[]> {
        return this.http.get<any[]>(`/api/v1/memory/query/entities/${entityId}/contexts`);
    }

    /** Get all entities mentioned in a context. */
    async getEntitiesForContext(contextId: string): Promise<Entity[]> {
        return this.http.get<Entity[]>(`/api/v1/memory/query/contexts/${contextId}/entities`);
    }
    ...
    async getHistoryByUser(uid: string): Promise<QueryResponse> {
        return this.http.get<QueryResponse>(`/api/v1/memory/query/history/user/${uid}`);
    }

    /** Get recent knowledge bases from the last N hours. */
    async getRecentKbs(hours: number = 24): Promise<QueryResponse> {
        return this.http.get<QueryResponse>("/api/v1/memory/query/history/recent", { hours });
    }

    /** Get knowledge bases since a timestamp (ISO-8601). */
    async getKbsSince(since: string): Promise<QueryResponse> {
        return this.http.get<QueryResponse>("/api/v1/memory/query/history/since", { since });
    }
    ...
    async deleteUserData(uid: string): Promise<void> {
        await this.http.delete<void>(`/api/v1/memory/query/user/${uid}`);
    }
    ...
    async getOutgoingConnections(entityId: string): Promise<QueryResponse> {
        return this.http.get<QueryResponse>(`/api/v1/memory/query/graph/outgoing/${entityId}`);
    }

    /** Get incoming relations for an entity. */
    async getIncomingConnections(entityId: string): Promise<QueryResponse> {
        return this.http.get<QueryResponse>(`/api/v1/memory/query/graph/incoming/${entityId}`);
    }

    /** Get 2-hop connections (entity names reachable in 2 hops). */
    async getTwoHopConnections(entityId: string): Promise<string[]> {
        return this.http.get<string[]>(`/api/v1/memory/query/graph/two-hop/${entityId}`);
    }

    /** Get top/strongest relations. */
    async getTopRelations(limit: number = 10): Promise<Relation[]> {
        return this.http.get<Relation[]>("/api/v1/memory/query/graph/top-relations", { limit });
    }

    /** Get relations by source entity. */
    async getRelationsBySource(sourceId: string): Promise<Relation[]> {
        return this.http.get<Relation[]>(`/api/v1/memory/query/graph/relations/source/${sourceId}`);
    }

    /** Get relations by target entity. */
    async getRelationsByTarget(targetId: string): Promise<Relation[]> {
        return this.http.get<Relation[]>(`/api/v1/memory/query/graph/relations/target/${targetId}`);
    }

    /** Get relations by type. */
    async getRelationsByType(relationType: string): Promise<Relation[]> {
        return this.http.get<Relation[]>(`/api/v1/memory/query/graph/relations/type/${relationType}`);
    }
    ...
    async routeQuery(query: string): Promise<QueryResponse> {
        return this.http.post<QueryResponse>("/api/v1/memory/query/route", { query, limit: 5, minRelevance: 0.7 });
    }
